...
func NewMaxHeap(elems []MaxHeapElem) *MaxHeap {
	heapElems := make([]MaxHeapElem, len(elems))
...
func (h *MaxHeap) FindMax() MaxHeapElem {
	return h.elems[0]
}

func (h *MaxHeap) ExtractMax() MaxHeapElem {
	result := h.elems[0]
	last := len(h.elems) - 1
...
func (h *MaxHeap) Insert(elem MaxHeapElem) int {
	i := len(h.elems)
...
func (h *MaxHeap) FindTop(k uint) []MaxHeapElem {
	result := make([]MaxHeapElem, 0, k)
	for i := uint(0); i < k; i++ {
...
func IsHeap(elems []MaxHeapElem) bool {
	for i := range elems {
		left := 2*i + 1
		right := 2*i + 2
